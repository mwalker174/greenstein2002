	      ...
	      double state[N],double current[Ncur])
{
  ...
  //double        FRU_states[Nstates_FRU][NFRU_sim_max];
  //double          FRU_states_hold[NFRU_sim_max][Nstates_FRU];
  //int             LType_state_hold[NFRU_sim_max][Nclefts_FRU][Nindepstates_LType];
  //int             RyR_state_hold[NFRU_sim_max][Nclefts_FRU][NRyRs_per_cleft];
  //int             Ito2_state_hold[NFRU_sim_max][Nclefts_FRU];
  ...
  //int           idall_ran1_hold[34, NFRU_sim_max]
  // int          idall_ran2_hold[35, NRVseqs_per_FRU, NFRU_sim_max]
  //int             mti_hold[NFRU_sim_max];
  
  //int           mt_hold[mtN - 1][NFRU_sim_max];
  //unsigned long   mt_hold[NFRU_sim_max][mtN];
  //int           mag01_hold[1][NFRU_sim_max];
  
  //double        surand_seed_hold[NFRU_sim_max]
  int             notaccepted, stepsno, forcedaccept;
  
  double          sum_CaSS, sum_CaJSR;
  ...
  while (notdone) {
    ...
    for (i = 0; i < N; i++) {
      ...
      k1[i] = step_size * F1[i];
      y_1[i] = state[i] + k1[i] / 3.0;
    }
    
    //Set estimated values for V,Cai, CaNSR at the end
    // of the intermediate RK4M time step so that these
    // quantites can be interpolated during the upcoming
    // Monte Carlo simulation(call to simfru)
    ...
    end_time = start_time + step_size / 3.0;
    ...
    fcn(start_time + step_size / 3.0, y_1, F, dummycurrent, dummyfalse, 
	Jxfer, Jtr, ICa, Ito2);
    
    for (i = 0; i < N; i++) {
      //Second intermediate RK4M step
      k2[i] = step_size * F[i];
      y_1[i] = state[i] + (k1[i] + k2[i])/6.0;
    }
    ...
    st_time = start_time + step_size / 3.0;

    // calc_fru_flux() here
    fcn(start_time + step_size / 3.0, y_1, F, dummycurrent, dummyfalse, Jxfer, Jtr, ICa, Ito2);
    
    for (i = 0; i < N; i++) {
      //Second intermediate RK4M step
      k2[i] = step_size * F[i];
      y_1[i] = state[i] + (k1[i] + 3.0*k3[i])/8.0;
    }
    ...
    FRUdep_states0[index_frudep_V] = y_1[index_V];
    FRUdep_states0[index_frudep_Cai] = y_1[index_Cai];
    FRUdep_states0[index_frudep_CaNSR] = y_1[index_CaNSR];

    fcn(start_time + step_size * 2.0, y_1, F, dummycurrent, dummyfalse, Jxfer, Jtr, ICa, Ito2);
    ...
    distrib_simFRU(st_time,end_time,FRUdep_states0,FRUdep_statesf,
		   &sum_CaSS, &sum_CaJSR, &Jxfer, &Jtr, &ICa, &Ito2);
    
    //DepFlag = 1;
    //calc_fru_flux(LType_state, Ito2_state, FRU_states, FRUdep_statesf, DepFlag,
    //	&sum_CaSS, &sum_CaJSR, &Jxfer, &Jtr, &ICa, &Ito2);
...
*/
double rkf4am_predict_step_size(double previous, double t, double tstep, 
                             double state[N],double current[Ncur],
